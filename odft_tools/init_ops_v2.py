...
class VarianceScaling(Initializer):
  ...
  where n is:
    - number of input units in the weight tensor, if mode = "fan"
  ...
  Args:
    ...
    mode: One of "fan", "fan_out", "fan_avg".
...
class GlorotUniform(VarianceScaling):
  ...
  is `sqrt(6 / (fan + fan_out))` where `fan` is the number of input units
...
class GlorotNormal(VarianceScaling):
  ...
  = sqrt(2 / (fan + fan_out))` where `fan` is the number of input units in
...
def lecun_normal(seed=None):
  ...
  being initialized.
  Draws samples from a truncated normal distribution centered on 0 with `stddev
  = sqrt(1 / fan)` where `fan` is the number of input units in the weight
  tensor.
  Examples:
  >>> def make_variables(k, initializer):
  ...   return (tf.Variable(initializer(shape=[k, k], dtype=tf.float32)),
  ...
  return VarianceScaling(
      scale=1., mode="fan", distribution="truncated_normal", seed=seed)
...
def lecun_uniform(seed=None):
  ...
  being initialized.
  Draws samples from a uniform distribution within [-limit, limit] where `limit`
  is `sqrt(3 / fan)` where `fan` is the number of input units in the
  weight tensor.
  Examples:
  >>> def make_variables(k, initializer):
  ...   return (tf.Variable(initializer(shape=[k, k], dtype=tf.float32)),
  ...
  """
  return VarianceScaling(
      scale=1., mode="fan", distribution="uniform", seed=seed)
...
def he_normal(seed=None):
  ...
  `stddev = sqrt(2 / fan)` where `fan` is the number of input units in the
  weight tensor.
  Examples:
  >>> def make_variables(k, initializer):
  ...   return (tf.Variable(initializer(shape=[k, k], dtype=tf.float32)),
  ...
  """
  return VarianceScaling(
      scale=2., mode="fan", distribution="truncated_normal", seed=seed)
...
def he_uniform(seed=None):
  ...
  being initialized.
  Draws samples from a uniform distribution within [-limit, limit] where `limit`
  is `sqrt(6 / fan)` where `fan` is the number of input units in the
  weight tensor.
  Examples:
  >>> def make_variables(k, initializer):
  ...   return (tf.Variable(initializer(shape=[k, k], dtype=tf.float32)),
  ...
  """
  return VarianceScaling(
      scale=2., mode="fan", distribution="uniform", seed=seed)
