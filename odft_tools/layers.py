...
class Continuous1DConvV1(keras.layers.convolutional.Conv1D):
    ...
    Arguments:
        ...
        weighs: list with mean at first place and stddev at second
        random_init: Boolean, if True random init. of means and stddvs
                     mean and stddev from weights are limit for random
                     uniform distributed means and stddevs for init.
                     gaussian kernel
    """
    # Costum Layer for continous convolution
    def __init__(self,
                 weights_init,
                 random_init=False,
                 seed=None,
...
class Harmonic1DConvV1(keras.layers.convolutional.Conv1D):
    """
    Arguments:
        filters: Integer, the dimensionality of the output space
            (i.e. the number of output filters in the convolution).
    ...
    def __init__(self,
        ...
        self.seed = seed
        # Set costum kernel init. with gaussian kernel
        self.kernel_initializer = TrigonometricsKernel1DV1(
            weights_init=weights_init,
            random_init=self.random_init,
            seed=self.seed
        )
...
# class Trigonometrics1DConvV1(keras.layers.convolutional.Conv1D):
#     """
#     Arguments:
#         filters: Integer, the dimensionality of the output space
#             (i.e. the number of output filters in the convolution).
...
#         self.seed = seed
#         # Set costum kernel init. with gaussian kernel
#         self.kernel_initializer = Trigonometrics(
#             weights_init=weights_init,
#             random_init=self.random_init,
#             seed=self.seed
#         )
